...
//    implements CellAttribute ,CellSpan  {
  ...
  /** This has been hacked a little to prevent it from blowing up
   *  when a row is added to an empty table. The five is hardcoded.
   *  I'll fix this someday. */
  public void addRow() {
    if(span.length == 0) {
      setSize(new java.awt.Dimension(5,1));
    } else {
      int[][][] oldSpan = span;
      int numRows    = oldSpan.length;
      int numColumns = oldSpan[0].length;
